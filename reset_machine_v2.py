...
def backup_file(file_path):
    ...
    if os.path.exists(file_path):
        ...
        logging.info('已创建备份文件:'+  backup_path)
...
def update_main_js(file_path):
    ...
    if not os.path.exists(file_path):
        logging.info('警告: main.js 文件不存在:' +  file_path)
        return False
    ...
    try:
        ...
        if success_marker in new_content:
            logging.info('main.js 文件修改成功')
            return True
        else:
            logging.info('警告: main.js 文件可能未被正确修改，请检查文件内容')
            logging.info('你可以从备份文件恢复:' + file_path + '.backup_*')
            return False

    except Exception as e:
        logging.error('修改 main.js 时出错:' +  str(e))
        return False
...
def reset_machine_id():
    ...
    try:
        ...
        logging.info('配置文件路径:' +  storage_path)
        ...
        # 输出结果
        logging.info('\n已成功修改 ID:')
        logging.info('machineId:' + machine_id)
        logging.info('macMachineId:' + mac_machine_id)
        logging.info('devDeviceId:' + dev_device_id)
